...
def createBoneShapes():
	...
	if (bpy.data.objects.get("effectorBone1") is None) or (bpy.data.objects.get("effectorBone2") is None):
		...
		bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=1, radius=0.5, enter_editmode=False)
		...
		effectorBone1.hide_viewport = True
		effectorBone2.hide_viewport = True
...
def addEffectorObj(objList, rig):
	...
	for obj in objList:
		...
		if (preParent):
			...
			preParent.select_set(state=True)
			empty.select_set(state=True)
			bpy.context.view_layer.objects.active = preParent
			bpy.ops.object.parent_set(type='OBJECT', keep_transform=True)
...
# note this should maybe be _OT_
...
class A123_PT_effectorPanel(bpy.types.Panel):
	"""Effector Tools"""
	...
	def draw(self, context):
		...
		# removed in blender 2.8 (https://docs.blender.org/api/current/change_log.html)
...
class WIPpopup(bpy.types.Operator):
	...
	def draw(self, context):
		self.layout.label(text="This addon is a work in progress, feature not yet implemented")
...
def register():
	...
	bpy.utils.register_class(A123_PT_effectorPanel)
...
def unregister():
	...
	bpy.utils.unregister_class(A123_PT_effectorPanel)
